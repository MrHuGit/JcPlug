...
/**
 * 作用：分页的网格布局管理器 作者：GcsSloop 摘要： 1. 网格布局 2. 支持水平分页和垂直分页 3. 杜绝高内存占用
 */
public class PagerGridLayoutManager extends RecyclerView.LayoutManager
     ...
     * 当适配器的数据改变时(或者整个适配器被换掉时)会再次调用。 它的作用就是在初始化的时候放置item，直到填满布局为止。
     * 1、在RecyclerView初始化时，会被调用两次。
     * 2、在调用adapter.notifyDataSetChanged()时，会被调用。
     * 3、 在调用setAdapter替换Adapter时,会被调用。
     * 4、在RecyclerView执行动画时，它也会被调用。 获取对应位置的子view 添加进RecyclerView 测量子View的宽高 根据测量的宽高,给他们排列好位置
     *
